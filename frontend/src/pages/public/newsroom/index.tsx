    ...
    <NewsroomShell>
      <main>
        {loading ? <div className="site-container py-24 text-center text-slate-500">Loading Newsroom…</div> : <>
          ...
          <section className="site-container pb-14"><div className="flex items-end justify-between gap-4"><div><p className="section-badge">Highlights</p><h2 className="mt-4 text-3xl font-black text-slate-950 dark:text-white">Latest from Medanfo</h2></div><Link to="/newsroom/news" className="hidden items-center gap-2 text-sm font-black text-[var(--brand-green)] sm:flex">All stories <ArrowRight size={16} /></Link></div><div className="mt-8 grid gap-6 lg:grid-cols-3">{latest.slice(0, 3).map((item) => <NewsroomCard key={item.id} item={item} />)}</div></section>
